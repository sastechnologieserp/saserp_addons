...
class ReceivablePayableReport:
	...
	def set_invoice_details(self, row):
		...
		if row.voucher_type == "Sales Invoice":
			...
			#Identify Sales Returns
			is_return = frappe.db.get_value("Sales Invoice", row.voucher_no, "is_return")
			row["is_return"] = 1 if is_return else 0

		elif row.voucher_type == "Purchase Invoice":
			#Identify Purchase Returns
			is_return = frappe.db.get_value("Purchase Invoice", row.voucher_no, "is_return")
			row["is_return"] = 1 if is_return else 0
